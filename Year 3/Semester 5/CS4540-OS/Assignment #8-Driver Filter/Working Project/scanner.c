...
--*/
{
	...
	//scanFile = ScannerpCheckExtension(&nameInfo->Name);
...
--*/
{
	...
	try {
		...
		if (Data->Iopb->Parameters.Write.Length != 0) {
			...
			//Here we have to test if this is the file to work on
			BOOLEAN doTheOperationThing = FALSE;
			try
			{
				//This does the comparison
				PWSTR testThingForFile = wcsstr(FltObjects->FileObject->FileName.Buffer, L"dos.txt");
				//if it's got something in it, that means it found something
				if (testThingForFile != NULL)
				{
					//make it true for us to use in a few lines
					doTheOperationThing = TRUE;
				}
			}
			...
			try 
			{
				...
				//so, if it's the file we want
				if (doTheOperationThing == TRUE)
				{
					//set up a buffer we can manipulate without destroying everything
					char* temporaryBufferToDoOperationOn = (char*)buffer;
					//find the length of the buffer for the loop
					int loopCounterForOperation = (int)strlen(temporaryBufferToDoOperationOn);
					//loop through all chars to find the ones we're looking for
					for (int i = 0; i < loopCounterForOperation; i++)
					{
						//if it's the DOS ending that's not in linux
						if (temporaryBufferToDoOperationOn[i] == '\r')
						{
							//make it a space
							temporaryBufferToDoOperationOn[i] = ' ';
						}
					}
					//now put the temporary buffer back into the original
					buffer = (PUCHAR)temporaryBufferToDoOperationOn;
